...
class Database:
    ...
    async def check_engineer_registered(self, user_id: int):
        ...
        async with self.pool.acquire() as conn:
            record = await conn.fetchrow(
                "SELECT id, full_name FROM engineers WHERE user_id = $1",
                user_id
            )
            return record
    
    async def save_application_with_type(self, application_name: str, thread_id: int, chat_id: int, application_type: str):
        ...
        async with self.pool.acquire() as conn:
            try:
                ...
                ''', application_name, thread_id, chat_id, application_type)
                logger.info(f"Заявка '{application_name}' типа '{application_type}' сохранена в БД")
                return True
            except Exception as e:
    ...
    async def save_labor_cost(self, thread_id: int, user_id: int, engineer_name: str, hours: float, message_text: str):
        """Сохранение трудозатрат в БД"""
        async with self.pool.acquire() as conn:
            try:
                # Получаем ID заявки по thread_id
                app_record = await conn.fetchrow(
                    "SELECT id FROM applications WHERE thread_id = $1",
                    thread_id
                )
                ...
                if app_record and engineer_record:
                    ...
                    ''', app_record['id'], thread_id, engineer_record['id'], user_id, engineer_name, hours, message_text)
...
async def handle_labor_cost(message: Message):
    ...
    if text.lower().startswith('тзт'):
        logger.info(f"Обнаружены трудозатраты от инженера {engineer_name} в топике {message.message_thread_id}")
            ...
            try:
                ...
                if application_name and application_type:
                    ...
                    if success:
                        ...
                        await message.reply(
                            "❌ Ошибка при сохранении трудозатрат. "
                            "Попробуйте позже."
                        )
                else:
                    await message.reply(
                ...
                await message.reply("❌ Не удалось распознать число часов. Используйте формат: тзт 2.5")
        else:
            await message.reply(
...
async def cmd_export_all(message: Message):
    ...
    status_msg = await message.answer("⏳ Подготавливаю Excel файл со всей статистикой...")
            ...
            with pd.ExcelWriter(filepath, engine='openpyxl') as writer:
                ...
                if not operational_df.empty:
                    ...
                    for app_name in operational_df['Название заявки'].unique():
                        app_df = operational_df[operational_df['Название заявки'] == app_name]
                        total_hours = app_df['Часы'].sum()
                        
                        engineers_info = []
                        for engineer in app_df['Инженер'].unique():
                            eng_hours = app_df[app_df['Инженер'] == engineer]['Часы'].sum()
                ...
                if not technical_df.empty:
                    ...
                    for app_name in technical_df['Название заявки'].unique():
                        app_df = technical_df[technical_df['Название заявки'] == app_name]
                        total_hours = app_df['Часы'].sum()
                ...
                # Лист 4: По инженерам
                pivot_by_engineer = df.groupby('Инженер').agg({
                    'Часы': 'sum',
                    'Название заявки': lambda x: len(set(x))
                }).rename(columns={'Часы': 'Всего часов', 'Название заявки': 'Количество заявок'})
...
async def cmd_export_current(message: Message):
    ...
    status_msg = await message.answer("⏳ Подготавливаю Excel файл со статистикой за текущий месяц...")
    ...
    try:
        ...
        async with db.pool.acquire() as conn:
            ...
            if not data:
                ...
                await status_msg.edit_text(f"📊 За {month_name} нет данных для экспорта")
                return
